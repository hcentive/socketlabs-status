...
exports.checkAccountData = function(username, password, serverid, callback) {
  ...
  // servers.forEach(function(server) {
    // limiter.submit(checkServer, server, u, p, sp, callback);
  // });

  // checkServer (server, u, p, callback);
  ...
  var req = https.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      if (json.result) {
        ...
        sp.nagiosExit(messageObj.state, messageObj.message);
  ...
  // req.end(() => {
  //   callback(null, "Server status retrieved - " + server);
  // });
};
