...
var conf = JSON.parse(fs.readFileSync(path.join('.', 'conf', 'default.json')));
...
exports.checkAccountData = function(cb) {
  var limiter = new Bottleneck(1, conf.api_time_to_wait_ms);

  conf.server_ids.forEach(function(server, callback) {
    limiter.submit(checkServer, server, function(e, r) { cb(e, r); });
  });
};
